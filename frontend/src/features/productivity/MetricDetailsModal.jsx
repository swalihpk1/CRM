      ...
      <div className="flex justify-between items-start gap-2">
        <div className="flex-1 min-w-0">
          ...
          {item.attendees && item.attendees.length > 0 && (
            <p className="text-sm text-gray-600 mt-1">👥 {item.attendees.length} attendee(s)</p>
          )}
        </div>
        <span
          className={`px-3 py-1 rounded-full text-xs font-semibold shrink-0 ${
        ...
        <span
          className={`px-3 py-1 rounded-full text-xs font-semibold shrink-0 ${
            item.contact?.status === 'Follow Up'
              ...
              ? 'bg-green-100 text-green-800'
              : item.contact?.status === 'Not Interested'
              ? 'bg-red-100 text-red-800'
              : item.contact?.status === 'Closed'
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] sm:max-h-[80vh] overflow-hidden">
        <div className="border-b px-6 py-4 flex justify-between items-center">
          <h2 className="text-base sm:text-xl font-bold text-gray-800">
            {staffName} - {METRIC_LABELS[metric] || metric}
          </h2>
          <button
            onClick={onClose}
