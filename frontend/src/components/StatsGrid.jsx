 ...
 *
 * `items`: [{ key, label, value, colorClass?, onClick? }] — an item with
 * onClick renders as a clickable/hoverable card (e.g. Dashboard's status
 * cards link through to the filtered Contacts list); items without it
 * stay plain, non-interactive cards exactly as before.
      ...
      {items.map((item) => {
        ...
        return (
          <Tag
            ...
            className={`bg-white rounded-xl shadow-sm border border-gray-200 px-3 py-3 sm:px-4 sm:py-4 text-left w-full ${
              item.onClick ? 'hover:shadow-md hover:border-indigo-200 transition cursor-pointer' : ''
            }`}
          >
