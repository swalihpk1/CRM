  ...
  return (
    <div className={`p-4 ${getRowStyling(log.action)}`}>
      <div className="flex justify-between items-center gap-2">
        ...
        <div className="flex items-center text-xs text-gray-700 shrink-0">
          <div className="w-2 h-2 bg-indigo-500 rounded-full mr-1.5" />
          {log.user_email.split('@')[0]}
        </div>
      </div>
      ...
      <div className="mt-2 text-xs text-gray-500 font-mono">{formatIndianDate(log.timestamp)}</div>
    </div>
  );
}
